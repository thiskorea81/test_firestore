...
import { Save, Plus, Trash2, ArrowUp, ArrowDown } from 'lucide-vue-next';

const store = useSystemStore();
const localConfig = ref(JSON.parse(JSON.stringify(store.config))); // 깊은 복사
...
const addApprovalStep = () => {
  localConfig.value.approvalLine.push({ role: '', label: '', isFinal: false });
};
const removeApprovalStep = (index) => {
  localConfig.value.approvalLine.splice(index, 1);
};
...
const handleSave = () => {
  store.saveConfig(localConfig.value);
};
</script>
  ...
  <div class="config-container">
    ...
    <div class="section">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="input-group">
          ...
          <input type="number" v-model="localConfig.limits.menstrual" class="input-base" />
          <span class="unit">일</span>
        </div>
        <div class="input-group">
          <label>국내 체험학습 / 연</label>
          <input type="number" v-model="localConfig.limits.domesticTrip" class="input-base" />
          <span class="unit">일</span>
        </div>
        <div class="input-group">
          <label>국외 체험학습 / 연</label>
          <input type="number" v-model="localConfig.limits.overseasTrip" class="input-base" />
          <span class="unit">일</span>
        </div>
      </div>
    </div>
    ...
    <div class="section">
      <div class="flex justify-between items-end mb-2">
        <h4 class="sub-title">📝 결재 라인 관리 (순서대로 표시됨)</h4>
      ...
      <div class="approval-list">
        <div v-for="(step, index) in localConfig.approvalLine" :key="index" class="approval-item">
          ...
          <input type="text" v-model="step.label" placeholder="직책 (예: 교장)" class="input-base w-32" />
    ...
    <div class="mt-6 flex justify-end">
      <button @click="handleSave" class="btn-save">
        <Save class="w-5 h-5 mr-2"/> 설정 저장하기
      </button>
    </div>
  </div>
</template>
...
.approval-item { display: flex; align-items: center; background: white; padding: 0.5rem 1rem; border-radius: 0.5rem; border: 1px solid #e5e7eb; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
